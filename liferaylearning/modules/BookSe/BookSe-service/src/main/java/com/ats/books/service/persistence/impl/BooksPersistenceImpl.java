...
import com.ats.books.model.impl.BooksImpl;
import com.ats.books.model.impl.BooksModelImpl;
...
public class BooksPersistenceImpl
	...
	private FinderPath _finderPathWithPaginationFindByByAuthor;
	private FinderPath _finderPathWithoutPaginationFindByByAuthor;
	private FinderPath _finderPathCountByByAuthor;
	 ...
	 */
	@Override
	public List<Books> findByByAuthor(String author) {
		return findByByAuthor(
	 ...
	 */
	@Override
	public List<Books> findByByAuthor(String author, int start, int end) {
	 ...
	 */
	@Override
	public List<Books> findByByAuthor(
		String author, int start, int end,
		OrderByComparator<Books> orderByComparator) {

		return findByByAuthor(author, start, end, orderByComparator, true);
	}
	...
	public List<Books> findByByAuthor(
		...
		if ((start == QueryUtil.ALL_POS) && (end == QueryUtil.ALL_POS) &&
			...
			if (useFinderCache) {
				finderPath = _finderPathWithoutPaginationFindByByAuthor;
				finderArgs = new Object[] {author};
			}
		}
		else if (useFinderCache) {
			finderPath = _finderPathWithPaginationFindByByAuthor;
		...
		if (list == null) {
			...
			if (author.isEmpty()) {
				sb.append(_FINDER_COLUMN_BYAUTHOR_AUTHOR_3);
			}
			else {
				bindAuthor = true;

				sb.append(_FINDER_COLUMN_BYAUTHOR_AUTHOR_2);
			}
	 ...
	 */
	@Override
	public Books findByByAuthor_First(
			String author, OrderByComparator<Books> orderByComparator)
		throws NoSuchBooksException {

		Books books = fetchByByAuthor_First(author, orderByComparator);
	 ...
	 */
	@Override
	public Books fetchByByAuthor_First(
		String author, OrderByComparator<Books> orderByComparator) {

		List<Books> list = findByByAuthor(author, 0, 1, orderByComparator);
	 ...
	 */
	@Override
	public Books findByByAuthor_Last(
			String author, OrderByComparator<Books> orderByComparator)
		throws NoSuchBooksException {

		Books books = fetchByByAuthor_Last(author, orderByComparator);
	 ...
	 */
	@Override
	public Books fetchByByAuthor_Last(
		String author, OrderByComparator<Books> orderByComparator) {
		...
		List<Books> list = findByByAuthor(
	 ...
	 */
	@Override
	public Books[] findByByAuthor_PrevAndNext(
		...
		try {
			...
			array[0] = getByByAuthor_PrevAndNext(
				session, books, author, orderByComparator, true);

			array[1] = books;

			array[2] = getByByAuthor_PrevAndNext(
				session, books, author, orderByComparator, false);
	...
	protected Books getByByAuthor_PrevAndNext(
		Session session, Books books, String author,
		OrderByComparator<Books> orderByComparator, boolean previous) {
	 ...
	 */
	@Override
	public void removeByByAuthor(String author) {
		for (Books books :
				findByByAuthor(
					author, QueryUtil.ALL_POS, QueryUtil.ALL_POS, null)) {
	 ...
	 */
	@Override
	public int countByByAuthor(String author) {
		author = Objects.toString(author, "");

		FinderPath finderPath = _finderPathCountByByAuthor;
	...
	private static final String _FINDER_COLUMN_BYAUTHOR_AUTHOR_2 =
		"books.author = ?";

	private static final String _FINDER_COLUMN_BYAUTHOR_AUTHOR_3 =
		"(books.author IS NULL OR books.author = '')";
	...
	public void cacheResult(List<Books> bookses) {
		if ((_valueObjectFinderCacheListThreshold == 0) ||
			((_valueObjectFinderCacheListThreshold > 0) &&
			 (bookses.size() > _valueObjectFinderCacheListThreshold))) {
	...
	public void clearCache() {
		...
		finderCache.clearCache(BooksImpl.class);
	}
	...
	protected void cacheUniqueFindersCache(BooksModelImpl booksModelImpl) {
		...
		finderCache.putResult(_finderPathFetchByUUID_G, args, booksModelImpl);
	}
	...
	public Books updateImpl(Books books) {
		...
		if (isNew && (books.getCreateDate() == null)) {
			if (serviceContext == null) {
				books.setCreateDate(date);
			}
			else {
				books.setCreateDate(serviceContext.getCreateDate(date));
			}
		}

		if (!booksModelImpl.hasSetModifiedDate()) {
			if (serviceContext == null) {
				books.setModifiedDate(date);
			}
			else {
				books.setModifiedDate(serviceContext.getModifiedDate(date));
			}
		}
	...
	public void activate() {
		...
		_finderPathWithPaginationFindByByAuthor = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByByAuthor",
			new String[] {
				String.class.getName(), Integer.class.getName(),
				Integer.class.getName(), OrderByComparator.class.getName()
			},
			new String[] {"author"}, true);

		_finderPathWithoutPaginationFindByByAuthor = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "findByByAuthor",
			new String[] {String.class.getName()}, new String[] {"author"},
			true);

		_finderPathCountByByAuthor = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByByAuthor",
			new String[] {String.class.getName()}, new String[] {"author"},
			false);

		BooksUtil.setPersistence(this);
	}

	@Deactivate
	public void deactivate() {
		BooksUtil.setPersistence(null);
